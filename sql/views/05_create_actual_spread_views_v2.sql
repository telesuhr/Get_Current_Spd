...
-- Version: 1.2 - Using correct field names
-- Date: 2025-07-18
...
SELECT 
    ...
    AVG(md.todays_volume) as avg_volume,
...
-- 5. Today's active spreads by actual type
CREATE OR ALTER VIEW lme_market.V_today_active_by_actual_type
AS
SELECT 
    m.metal_code,
    COALESCE(s.actual_spread_type, s.spread_type) as spread_type,
...
FROM lme_market.LME_M_spreads s
JOIN lme_config.LME_M_metals m ON s.metal_id = m.metal_id
JOIN lme_market.V_latest_market_data md ON s.ticker = md.ticker
WHERE s.is_active = 1
  AND md.last_update_dt >= CAST(GETDATE() AS DATE);
GO

-- 6. Summary of spread types comparison
CREATE OR ALTER VIEW lme_market.V_spread_classification_summary
AS
SELECT 
    m.metal_code,
    s.spread_type as original_type,
    ...
    COUNT(*) as count,
    CASE 
        WHEN s.spread_type = s.actual_spread_type THEN 'Matched'
...
WHERE s.is_active = 1
  AND s.actual_spread_type IS NOT NULL
GROUP BY m.metal_code, s.spread_type, s.actual_spread_type;
GO

PRINT 'Created views using actual spread types successfully';
GO