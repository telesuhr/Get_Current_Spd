...
SELECT 
    ...
    s.leg1_description,
    s.leg2_description,
    md.last_bid_price as bid_price,
    md.last_ask_price as ask_price,
    md.last_trade_price as last_price,
    md.today_volume as volume,
    md.last_update_dt,
...
SELECT 
    ...
    AVG(md.today_volume) as avg_volume,
...
-- 3. Cash-3W spreads specifically (without ORDER BY)
...
FROM lme_market.LME_M_spreads s
JOIN lme_config.LME_M_metals m ON s.metal_id = m.metal_id
JOIN lme_market.V_latest_market_data md ON s.spread_id = md.spread_id
WHERE s.is_active = 1
