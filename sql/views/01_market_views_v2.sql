-- Market Data Views (Updated for new table names)
-- Version: 2.0
-- Date: 2025-07-18
...
JOIN market.T_tick_data t ON t.spread_id = lt.spread_id AND t.timestamp = lt.latest_timestamp
WHERE s.is_active = 1;
GO
...
FROM market.M_spreads s
JOIN config.M_metals m ON s.metal_id = m.metal_id
LEFT JOIN market.T_tick_data t ON s.spread_id = t.spread_id
WHERE s.is_active = 1
...
FROM config.M_metals m
JOIN RecentData rd ON m.metal_id = rd.metal_id
LEFT JOIN config.M_collection_config cc ON m.metal_id = cc.metal_id
WHERE m.is_active = 1;
GO

-- View for daily summary data
CREATE OR ALTER VIEW market.v_daily_summary
AS
SELECT 
    m.metal_code,
    s.ticker,
...
FROM market.T_daily_summary ds
JOIN market.M_spreads s ON ds.spread_id = s.spread_id
JOIN config.M_metals m ON s.metal_id = m.metal_id;
GO

PRINT 'Views updated successfully for new table names';