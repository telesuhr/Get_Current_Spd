-- Stored Procedures for Data Insertion (Updated for new table names)
-- Version: 2.0
-- Date: 2025-07-18
...
BEGIN
    ...
    -- Insert tick data
    INSERT INTO market.T_tick_data (
...
IF EXISTS (SELECT * FROM sys.types WHERE name = 'TickDataType' AND schema_id = SCHEMA_ID('market'))
    DROP TYPE market.TickDataType;
GO
...
BEGIN
    ...
    WHERE NOT EXISTS (
        SELECT 1 
        FROM market.T_tick_data existing
...
BEGIN
    ...
    WITH DaySummary AS (
        ...
        JOIN market.M_spreads s ON t.spread_id = s.spread_id
    ...
    MERGE market.T_daily_summary AS target
    USING (
        SELECT 
            ds.spread_id,
            ds.trading_date,
...
CREATE OR ALTER PROCEDURE market.sp_GetActiveSpreads
    @metal_code NVARCHAR(10),
    @hours INT = 1
AS
BEGIN
...
PRINT 'Procedures updated successfully for new table names';